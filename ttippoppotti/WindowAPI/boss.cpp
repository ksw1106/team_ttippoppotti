...
HRESULT boss::init(float x, float y)
{
	...
	_terrorKopter.img.bodyImage[B_IDLE] = IMAGEMANAGER->findImage("테러콥터 아이들");
	_terrorKopter.img.bodyImage[B_TURN] = IMAGEMANAGER->findImage("테러콥터 회전");
	_terrorKopter.img.bodyImage[B_DEATH] = IMAGEMANAGER->findImage("테러콥터 시체");
	
	_terrorKopter.img.gunImage[BB_READY] = IMAGEMANAGER->findImage("테러콥터 기관총 예열");
	_terrorKopter.img.gunImage[BB_ROTATE] = IMAGEMANAGER->findImage("테러콥터 기관총 회전");
	_terrorKopter.img.gunImage[BB_FIRE] = IMAGEMANAGER->findImage("테러콥터 기관총 발사");	

	_terrorKopter.img.propellerImage = IMAGEMANAGER->findImage("테러콥터 프로펠러");
	_terrorKopter.img.bulletFireImage = IMAGEMANAGER->findImage("테러콥터 총알 효과");
