...
void playerManager::update(void)
{
	...
	if (COLLISIONMANAGER->pixelCollision(_img, tempX, tempY, _player->getSpeed(), _player->getGravity(), PLAYER_BOTTOM))		//바닥
	{
	...
	if (COLLISIONMANAGER->pixelCollision(_img, tempX, tempY, _player->getSpeed(), 0, PLAYER_RIGHT))				// 왼쪽벽
	{
		hit_left = true;
		_player->setIsJump(false);
		_player->setGravity(0.f);
	...
	if (COLLISIONMANAGER->pixelCollision(_img, tempX, tempY, _player->getSpeed(), 0, PLAYER_LEFT))				// 오른쪽벽
	{
		hit_right = true;
		_player->setIsJump(false);
		_player->setIsLeft(true);
