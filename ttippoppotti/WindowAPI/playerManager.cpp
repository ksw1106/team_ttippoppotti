...
void playerManager::update(void)
{
	...
	*/
	...
	{
		if (COLLISIONMANAGER->pixelCollision(_rcKnifeRight, knifeRightX, knifeRightY, _player->getSpeed(), _player->getGravity(), PLAYER_RIGHT))		// 칼빵 오른쪽 벽 뽀개기
		{
		...
		else if (COLLISIONMANAGER->pixelCollision(_rcKnifeLeft, knifeLeftX, knifeLeftY, _player->getSpeed(), _player->getGravity(), PLAYER_LEFT))		// 칼빵 왼쪽 벽 뽀개기
		{
			for (int i = 0; i < _mapData->getObject().size(); i++)
			{
